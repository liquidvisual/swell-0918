...
(function ($) {
    ...
    function getBreakpoints() {
        var $tableElement;
        var tableString = "";
        var bodyMetaString = window
            .getComputedStyle(document.getElementsByTagName("body")[0])
            .getPropertyValue("content");
        var breakpointsArray = bodyMetaString
            .replace(/\\a/g, "")
            .replace(/ /g, "")
            .replace(/'/g, "")
            .replace(/"/g, "")
        ...
        $tableElement =
            '<table class="table breakpoints-table" style="width: 300px;">' +
            tableString +
            "</table>";
    ...
    function responsiveHelper() {
        ...
        if (screenWidth >= breakpoints.xl) chromeScreen = "xl";

        if (queryWidth < breakpoints.sm) safariScreen = "xs";
        if (queryWidth >= breakpoints.sm && queryWidth < breakpoints.md)
            safariScreen = "sm";
        if (queryWidth >= breakpoints.md && queryWidth < breakpoints.lg)
            safariScreen = "md";
        if (queryWidth >= breakpoints.lg && queryWidth < breakpoints.xl)
            safariScreen = "lg";
        // if (queryWidth >= breakpoints.xl && queryWidth < breakpoints.xxl) safariScreen = "xl";
        // if (queryWidth >= breakpoints.xxl) safariScreen = "xxl";
        if (queryWidth >= breakpoints.xl) safariScreen = "xl";
